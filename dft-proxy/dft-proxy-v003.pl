...
# Input       : That's Top Secret
# Output      : This is too
# Dependencies: All sorts of IO stuff and POSIX stuff
# Description : Script to act as a "go-between" between the user(s) and
#               a misbehaving trouble ticketing system. =)
#               BUT, this has changed. It can be used as a proxy for almost
#               anything. CWL
# Usage       : Please see your nearest Fortune Teller
# Contact     : mdmonk@dftech.org
# Notes       : I prefer POP3 or IMAP rather than Notes.
# Keywords    : DFT
#############################################################################
# Copyright (C) 1999-2001, DFT
#
...
while (1) {
  ...
  # run waiting commands in the queue that are over $waitTime old
...
sub generateSID {
  ...
  $length = 8 if ($length != 8);  # added date to beginning to help make the key more
                                  # unique. so date with random chars equal 20 chars total.
  my $returnKey;
...
END
	...
	printMessage($peer, $sessionID, "#################\n");
      ...
      if (${*$peer}{__attributes}{qqnum}) {
        &{$command->{QQSTATUS}}($listen, $peer, $sessionID, @args);
      } # end if
    ...
    'QQSTATUS' => sub ($$$@) {
      ...
      my %statusHash = (rule=>'RS_CallStatus', qqnum=>"${*$peer}{__attributes}{qqnum}");
      my $qq = DFT::QQ::initiateQQ(%statusHash);
      while( ($key, $value) = each %{$qq}) {
